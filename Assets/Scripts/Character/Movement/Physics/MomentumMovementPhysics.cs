...
[System.Serializable]
public class MomentumMovementPhysicsValues : CharacterOverridableValues
{
    /// <summary>
...
/// </summary>
public class MomentumMovementPhysics : CharacterMovementPhysics
{
    ...
    [SerializeField]
    public CharacterOverridableAttribute<MomentumMovementPhysicsValues> overridableAttribute = new CharacterOverridableAttribute<MomentumMovementPhysicsValues>();
    ...
    public override void UpdateVelocity(ref Vector3 currentVelocity, float deltaTime)
    {
        if (movement.isGroundedThisUpdate) {
            ...
            if (Vector3.Dot(movement.groundNormal, gravityDirection) > 0)
            {
            ...
            float planePerpGravPercent = Mathf.Abs(Vector3.Dot(movement.currentPlane.normal, gravityDirection));
            if (planePerpGravPercent > movement.maxSlopeAngle/90)
            {
            ...
            if(overridableAttribute.values.constantAcceleration != Vector3.zero)
                currentVelocity += Vector3.ProjectOnPlane(overridableAttribute.values.constantAcceleration, movement.groundNormal) * deltaTime;
